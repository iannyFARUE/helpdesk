...
if(isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['hash']) && !empty($_GET['hash'])){
	...
	$sql = "SELECT * FROM md_users WHERE email = '$email' AND hash = '$hash'";
	$result = mysqli_query($conn, $sql);

	if(mysqli_num_rows($result) == 0){
		echo "<script>swal('Reset status','You have entered invalid url for password reset','error')
											.then((value) => {
	...
	//$_SESSION['message'] = "Sorry Email verification failed ";
echo "<script>swal('Reset status','Sorry Email verification failed ','error')
											.then((value) => {
									 		 window.location.href = \"index.php\";
      ...
      <form method="post" action="reset_password.php">
          ...
          <div class="row">
			...
			<div class="card-body">
				...
				<input type="password" name="verifypassword" class="form-control" placeholder="confirm password"><br>

				<!-- This input field is needed, to get the email of the user -->
				 <input type="hidden" name="email" value="<?= $email ?>">    
         		 <input type="hidden" name="hash" value="<?= $hash ?>"> 
			 
			<input type="submit" name="btnreset"  value="Sign in" id="resetBtn" class="btn btn-primary"><br><br>
			</div>
			</div>	
		</div>
		</div>
